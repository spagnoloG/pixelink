from flask import Flask, request, g, jsonify, render_template_string, make_response
...
# freeipa
app.config["FREEIPA_SERVER"] = "ipa.kp.org"
app.config["FREEIPA_USERNAME"] = "admin"
app.config["FREEIPA_PASSWORD"] = "admin12345"

freeipa_client = Client(app.config["FREEIPA_SERVER"])
freeipa_client.login(app.config["FREEIPA_USERNAME"], app.config["FREEIPA_PASSWORD"])
        ...
        if user_id:
            ...
            if user is None:
                ...
                if "application/json" in accept:
                    ...
                    return (
                        dicttoxml.dicttoxml(response_data).decode(),
                        404,
                    ...
                    return (
                        render_template_string(
                            template, message=response_data["message"]
                        ),
                        404,
            ...
            if "application/json" in accept:
                ...
                return (
                    dicttoxml.dicttoxml(user).decode(),
                    200,
                    {"Content-Type": "application/xml"},
                )
            elif "text/html" in accept:
            ...
            if "application/json" in accept:
                ...
                return (
                    dicttoxml.dicttoxml(all_users).decode(),
                    200,
                    {"Content-Type": "application/xml"},
                )
            elif "text/html" in accept:
                template = (
                    "<html><body><h1>All Users</h1><p>{{ users }}</p></body></html>"
                )
                return (
                    render_template_string(template, users=all_users),
                    200,
                    {"Content-Type": "text/html"},
                )
        return {"message": "Unsupported Media Type"}, 415
...
class MessageAPI(Resource):
    def get(self, message_id=None):
        ...
        print(g)
        if "ipa_session" not in request.cookies and not "user" in g:
...
def login():
    ...
    try:
        ...
        cookies = freeipa_client._session.cookies.get_dict()
        for name, value in cookies.items():
            response.set_cookie(name, value)
        return response
    except Exception as e:
        # Authentication failed
        return "Authentication failed: {}".format(str(e))
